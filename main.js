...
const h2TeksBot = document.createTextNode("GPT Bot");
...
function checkName() {
  if (!bannerInput.value) return alert("Please Enter Your Name.");
  containerBanner.style.display = "none";

  const newTeksBot = document.createTextNode(
    `Hello ${bannerInput.value}, Say hello to Chat Ai!`
  );
...
function clickSend() {
  ...
  const h2TeksBot2 = document.createTextNode("GPT Bot");
